<x-app-layout :meta-title="__('site.news')" meta-description="">
    ...
    <div class="mx-2">
        ...
        <div class="mb-4 flex rounded-md overflow-hidden border-2">
                ...
                <img class="w-60 object-cover" src="{{ $item->getThumbnail() }}" alt="post">
         
            <div class="p-4 grow">
                <h1 class="pb-4 border-b text-xl font-semibold">{{ $item->{'title_'.app()->getLocale()} }}</h1>
                <p class="text-sm text-gray-600">{{ $item->getFormattedDate() }} </p>
                <p class="mt-4 break-all font-tahoma">{!! $item->shortBody() !!}</p>
                <a class="block text-right underline my-2 text-gray-600" href="{{ route('news.view',['locale'=>app()->getLocale(),'news'=>$item]) }}">{{ __("site.more") }}</a>
            </div>
        </div>
        @endforeach
        <div>
